          ...
          function geocoder(message,type){
            ...
            geocoderS.getLocation(message, function(status, result) {
              if (status === 'complete' && result.info === 'OK') {
                if(type == 1){
                  _this.carList.startJ = (result.geocodes[0].location.lng).toString();
                  _this.carList.startW = result.geocodes[0].location.lat.toString();
                  geocoder(dataEnd,2);
                }else if(type == 2){
                  _this.carList.endJ = result.geocodes[0].location.lng.toString();
                  _this.carList.endW = result.geocodes[0].location.lat.toString();
